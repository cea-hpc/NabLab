...
import java.util.ArrayList;
		...
		public final class Glace2d
		{
			...
			private void iniCenter() 
			{
				...
				{
					int jId = jCells;
					Real2 sum1246914139 = new Real2(0.0, 0.0);
					...
					for (int rNodesOfCellJ=0; rNodesOfCellJ<nodesOfCellJ.length; rNodesOfCellJ++)
					{
						...
						int rNodes = rId;
						sum1246914139.operator_set(sum1246914139.operator_plus((coord[rNodes])));
					}
					center[jCells].operator_set(sum1246914139.operator_multiply((1.0 / 4.0)));
				});
			}		
			...
			private void iniVIc() 
			{
				...
				{
					int jId = jCells;
					double sum644993232 = 0.0;
					int[] nodesOfCellJ = mesh.getNodesOfCell(jId);
					for (int rNodesOfCellJ=0; rNodesOfCellJ<nodesOfCellJ.length; rNodesOfCellJ++)
					{
						int rId = nodesOfCellJ[rNodesOfCellJ];
						int rNodes = rId;
						sum644993232 = sum644993232 + (MathFunctions.dot(C_ic[jCells][rNodesOfCellJ], coord[rNodes]));
					}
					V_ic[jCells] = 0.5 * sum644993232;
				});
			}		
			...
			private void init_ComputeEn() 
			{
				IntStream.range(0, nbCells).parallel().forEach(j0Cells -> 
				{
					E[j0Cells] = p_ic[j0Cells] / ((options.gamma - 1.0) * rho_ic[j0Cells]);
				});
			}		
			...
			private void computeV() 
			{
				...
				{
					int jId = jCells;
					double sum170543614 = 0.0;
					int[] nodesOfCellJ = mesh.getNodesOfCell(jId);
					for (int rNodesOfCellJ=0; rNodesOfCellJ<nodesOfCellJ.length; rNodesOfCellJ++)
					{
						int rId = nodesOfCellJ[rNodesOfCellJ];
						int rNodes = rId;
						sum170543614 = sum170543614 + (MathFunctions.dot(C[jCells][rNodesOfCellJ], X[rNodes]));
					}
					V[jCells] = 0.5 * sum170543614;
				});
			}		
			...
			private void computedeltatj() 
			{
				...
				{
					int jId = jCells;
					double sum2092224846 = 0.0;
					int[] nodesOfCellJ = mesh.getNodesOfCell(jId);
					for (int rNodesOfCellJ=0; rNodesOfCellJ<nodesOfCellJ.length; rNodesOfCellJ++)
					{
						sum2092224846 = sum2092224846 + (l[jCells][rNodesOfCellJ]);
					}
					deltatj[jCells] = 2.0 * V[jCells] / (c[jCells] * sum2092224846);
				});
			}		
			...
			private void computeAr() 
			{
				...
				{
					int rId = rNodes;
					Real2x2 sum723805322 = new Real2x2(new Real2(0.0, 0.0), new Real2(0.0, 0.0));
					...
					for (int jCellsOfNodeR=0; jCellsOfNodeR<cellsOfNodeR.length; jCellsOfNodeR++)
					{
						...
						sum723805322.operator_set(sum723805322.operator_plus((Ajr[jCells][rNodesOfCellJ])));
					}
					Ar[rNodes].operator_set(sum723805322);
				});
			}		
			...
			private void computeBr() 
			{
				...
				{
					int rId = rNodes;
					Real2 sum870136207 = new Real2(0.0, 0.0);
					int[] cellsOfNodeR = mesh.getCellsOfNode(rId);
					for (int jCellsOfNodeR=0; jCellsOfNodeR<cellsOfNodeR.length; jCellsOfNodeR++)
					{
						int jId = cellsOfNodeR[jCellsOfNodeR];
						...
						int rNodesOfCellJ = Utils.indexOf(nodesOfCellJ,rId);
						int jCells = jId;
						sum870136207.operator_set(sum870136207.operator_plus((C[jCells][rNodesOfCellJ].operator_multiply(p[jCells]).operator_plus(Glace2dFunctions.matVectProduct(Ajr[jCells][rNodesOfCellJ], uj[jCells])))));
					}
					b[rNodes].operator_set(sum870136207);
				});
			}		
			...
			{
				double reduceMin2087332615 = IntStream.range(0, nbCells).boxed().parallel().reduce(
					Double.MAX_VALUE, 
					(r, jCells) -> MathFunctions.reduceMin(r, deltatj[jCells]),
					(r1, r2) -> MathFunctions.reduceMin(r1, r2)
				);
				deltat_n_plus_1 = options.option_deltat_cfl * reduceMin2087332615;
			}		
			...
			private void compute_ComputeUn() 
			{
				...
				{
					int jId = jCells;
					Real2 sum1880621155 = new Real2(0.0, 0.0);
					int[] nodesOfCellJ = mesh.getNodesOfCell(jId);
					for (int rNodesOfCellJ=0; rNodesOfCellJ<nodesOfCellJ.length; rNodesOfCellJ++)
					{
						sum1880621155.operator_set(sum1880621155.operator_plus((F[jCells][rNodesOfCellJ])));
					}
					uj_n_plus_1[jCells].operator_set(uj[jCells].operator_minus(sum1880621155.operator_multiply((deltat / m[jCells]))));
				});
			}		
			...
			private void compute_ComputeEn() 
			{
				...
				{
					int jId = jCells;
					double sum236575225 = 0.0;
					int[] nodesOfCellJ = mesh.getNodesOfCell(jId);
					for (int rNodesOfCellJ=0; rNodesOfCellJ<nodesOfCellJ.length; rNodesOfCellJ++)
					{
						int rId = nodesOfCellJ[rNodesOfCellJ];
						int rNodes = rId;
						sum236575225 = sum236575225 + (MathFunctions.dot(F[jCells][rNodesOfCellJ], ur[rNodes]));
					}
					E_n_plus_1[jCells] = E[jCells] - (deltat / m[jCells]) * sum236575225;
				});
			}		
