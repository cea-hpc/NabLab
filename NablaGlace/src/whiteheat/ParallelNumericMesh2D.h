...
namespace nablalib
{
	struct Real2
	{
	  ...
	  Real2& operator+=(const Real2& v) { x += v.x; y += v.y; return *this; }
	  Real2 operator*(const Real2& v) { return Real2(x*v.x, y*v.y); }
	  Real2 operator-(const Real2& v) { return Real2(x-v.x, y-v.y); }
	  Real2 operator+(const Real2& v) { return Real2(x+v.x, y+v.y); }
	  Real2 operator/(const Real2& v) { return Real2(x/v.x, y/v.y); }
	};
	...
	struct Real3
	{
	  ...
	  Real3& operator=(const Real3& v) { x = v.x; y = v.y; z = v.z; }
	  Real3& operator+=(const Real3& v) { x += v.x; y += v.y; z += v.z; }
	};
	...
	struct Real2x2
	{
	  ...
	  Real2x2& operator=(const Real2x2& v) { x = v.x; y = v.y; }
	  Real2x2& operator+=(const Real2x2& v) { x += v.x; y += v.y; }
	};
	...
	struct Real3x3
	{
	  ...
	  Real3x3& operator=(const Real3x3& v) { x = v.x; y = v.y; z = v.z; }
	  Real3x3& operator+=(const Real3x3& v) { x += v.x; y += v.y; z += v.z; }
	};
...
//			getFace(AId1).edgeIds(facesOfCell2);
	...
	double dot(const Real2& a, const Real2& b)
	{
		return (a.x*b.x) + (a.y*b.y);
	}

	double norm(const Real2& a)
	{
		return sqrt(dot(a,a));
	}

	double det(const Real2& a, const Real2& b)
	{
