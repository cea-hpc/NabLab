...
public final class Glace2d
{
	...
	private void iniCenter() 
	{
		...
		{
			int jId = jCells;
			Real2 sum134792494 = new Real2(0.0, 0.0);
			...
			for (int rNodesOfCellJ=0; rNodesOfCellJ<nodesOfCellJ.length; rNodesOfCellJ++)
			{
				...
				int rNodes = rId;
				sum134792494.operator_set(sum134792494.operator_plus((coord[rNodes])));
			}
			center[jCells].operator_set(sum134792494.operator_multiply((1.0 / 4.0)));
		});
	}		
	...
	private void computeAr() 
	{
		...
		{
			int rId = rNodes;
			Real2x2 sum360321736 = new Real2x2(new Real2(0.0, 0.0), new Real2(0.0, 0.0));
			...
			for (int jCellsOfNodeR=0; jCellsOfNodeR<cellsOfNodeR.length; jCellsOfNodeR++)
			{
				...
				sum360321736.operator_set(sum360321736.operator_plus((Ajr[jCells][rNodesOfCellJ])));
			}
			Ar[rNodes].operator_set(sum360321736);
		});
	}		
	...
	private void computeBr() 
	{
		...
		{
			int rId = rNodes;
			Real2 sum2077374394 = new Real2(0.0, 0.0);
			int[] cellsOfNodeR = mesh.getCellsOfNode(rId);
			for (int jCellsOfNodeR=0; jCellsOfNodeR<cellsOfNodeR.length; jCellsOfNodeR++)
			{
				int jId = cellsOfNodeR[jCellsOfNodeR];
				...
				int[] nodesOfCellJ = mesh.getNodesOfCell(jId);
				int rNodesOfCellJ = Utils.indexOf(nodesOfCellJ,rId);
				sum2077374394.operator_set(sum2077374394.operator_plus((C[jCells][rNodesOfCellJ].operator_multiply(p[jCells]).operator_plus(Glace2dFunctions.matVectProduct(Ajr[jCells][rNodesOfCellJ], uj[jCells])))));
			}
			b[rNodes].operator_set(sum2077374394);
		});
	}		
	...
	{
		double reduceMin1277006447 = IntStream.range(0, nbCells).boxed().parallel().reduce(
			Double.MAX_VALUE, 
	...
	private void compute_ComputeUn() 
	{
		...
		{
			int jId = jCells;
			Real2 sum2071597981 = new Real2(0.0, 0.0);
			int[] nodesOfCellJ = mesh.getNodesOfCell(jId);
			for (int rNodesOfCellJ=0; rNodesOfCellJ<nodesOfCellJ.length; rNodesOfCellJ++)
			{
				sum2071597981.operator_set(sum2071597981.operator_plus((F[jCells][rNodesOfCellJ])));
			}
			uj_n_plus_1[jCells].operator_set(uj[jCells].operator_minus(sum2071597981.operator_multiply((deltat / m[jCells]))));
		});
	}		
