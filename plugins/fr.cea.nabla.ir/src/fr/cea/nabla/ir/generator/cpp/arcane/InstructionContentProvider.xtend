...
import static extension fr.cea.nabla.ir.generator.cpp.arcane.ExpressionContentProvider.*
...
class InstructionContentProvider
{
	...
	'''
		Parallel::Foreach(«getContainerContent(iterator.container)», [&](«iterator.container.itemType.name.toFirstUpper»VectorView view)
		{
	...
	'''
		ENUMERATE_«iterator.container.itemType.name.toUpperCase»(«iterator.index.itemName», «getContainerContent(iterator.container)»)
		{
			«loopBody.innerContent»
		}
	'''
	...
	private static def getContainerContent(Container c)
	{
		switch c
		{
			ConnectivityCall: c.accessor
			SetRef: c.uniqueName
		}
	}
}
