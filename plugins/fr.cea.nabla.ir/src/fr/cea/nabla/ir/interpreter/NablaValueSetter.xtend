...
class NablaValueSetter
{
	...
	private static def dispatch subSet(NVVector it, int indice, NV0Real value) { setValue(indice, value.data) }
	...
	private static def dispatch subSet(NVMatrix it, int i, int j, NV0Real value) { setValue(i, j, value.data) }
