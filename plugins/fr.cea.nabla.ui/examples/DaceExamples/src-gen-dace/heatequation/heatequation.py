...
class HeatEquation:
	...
	def _iniCenter(self):
		...
		for jCells in range(startIndex, self._nbCells):
			...
			for rNodesOfCellJ in range(startIndex, nbNodesOfCellJ):
				rId = nodesOfCellJ[rNodesOfCellJ]
				rNodes = rId
				reduction0 = self._sumR1(reduction0, self.X[rNodes])
	...
	@dace.method
	def _operatorAdd(self, a, b):
		result = np.empty((a.size), dtype=np.double)
		startIndex = 0
	...
	def _operatorMult(self, a, b):
		result = np.empty((b.size), dtype=np.double)
		startIndex = 0
		for ix0 in range(startIndex, b.size):
			result[ix0] = a * b[ix0]
		return result
