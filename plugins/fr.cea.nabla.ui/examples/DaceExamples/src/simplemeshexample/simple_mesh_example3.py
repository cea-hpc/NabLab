...
class SimpleMeshExample:
    ...
    def simulate(self):
        ...
        cst = np.full(self.__nbNodes, 0.0)
        cst = np.array(cst)
        cst.astype(np.float64)
        
        nodesOfCells = np.empty((0,4), np.int64)
        for jCells in range(self.__nbCells):
            jId = jCells
        ...
        print("cst = ", self.cst)
        self._initTime() # @1.0
        self._computeSum(nodesOfCells=nodesOfCells, cst=self.cst, nodes_sum=self.nodes_sum) # @2.0
        print("nodes_sum = ", self.nodes_sum)
