...
IterativeHeatEquation::IterativeHeatEquation(const Options& aOptions)
: options(aOptions)
, t_n(0.0)
, t_nplus1(0.0)
, deltat(0.001)
...
void IterativeHeatEquation::computeResidual(const member_type& teamMember) noexcept
{
	...
	}, KokkosJoiner<double>(reduction0, -numeric_limits<double>::max(), &maxR0));
	residual = reduction0;
}
 ...
 * Out variables: xc, yc
 */
void IterativeHeatEquation::initXcAndYc(const member_type& teamMember) noexcept
{
	{
		const auto teamWork(computeTeamWorkRange(teamMember, nbCells));
		if (!teamWork.second)
...
int main(int argc, char* argv[]) 
{
	...
	IterativeHeatEquation::Options options(dataFile);
	// simulator must be a pointer if there is a finalize at the end (Kokkos, omp...)
	auto simulator = new IterativeHeatEquation(options);
	simulator->simulate();
