...
export default class LatexWebViewLoader {
  ...
  public static createOrShow(extensionPath: string) {
    ...
    const panel = vscode.window.createWebviewPanel('nablabweb', 'LaTex', column || vscode.ViewColumn.One, {
      enableScripts: true,
      localResourceRoots: [vscode.Uri.file(path.join(extensionPath, 'nablabweb'))],
    });
  ...
  public static getWebviewContent(webView: vscode.Webview, webViewContext: LatexWebViewContext): string {
    ...
    const reactAppPathOnDisk = vscode.Uri.file(path.join(webViewContext.extensionPath, 'nablabweb', 'nablabweb.js'));
