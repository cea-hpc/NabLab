...
Glace2d::Glace2d(gmds::Mesh& aMesh)
: mesh(aMesh)
, nbNodes(mesh.getNbNodes())
, nbCells(mesh.getNbFaces())
, nbInnerNodes(mesh.getGroup<gmds::Node>("InnerNodes")->size())
, nbTopNodes(mesh.getGroup<gmds::Node>("TopNodes")->size())
, nbBottomNodes(mesh.getGroup<gmds::Node>("BottomNodes")->size())
, nbLeftNodes(mesh.getGroup<gmds::Node>("LeftNodes")->size())
, nbRightNodes(mesh.getGroup<gmds::Node>("RightNodes")->size())
, X_n(nbNodes)
, X_nplus1(nbNodes)
, X_n0(nbNodes)
, b(nbNodes)
...
Glace2d::jsonInit(const char* jsonContent)
{
	...
	lastDump = std::numeric_limits<int>::min();
	n = 0;
	...
	for(auto i: mesh.nodes())
	{
		gmds::Node ni = mesh.get<gmds::Node>(i);
		X_n0[ni.id()][0] = ni.X();
		X_n0[ni.id()][1] = ni.Y();
    }
}
...
void Glace2d::computeCjr() noexcept
{
	...
	{
		const gmds::TCellID jId(jCells);
		const gmds::Face jCellsObj = mesh.get<gmds::Face>(jId);
		{
			const auto nodesOfCellJ = jCellsObj.getIDs<gmds::Node>();
			const int nbNodesOfCellJ(nodesOfCellJ.size());
			for(int rNodesOfCellJ=0; rNodesOfCellJ<nbNodesOfCellJ; rNodesOfCellJ++)
			{
				const gmds::TCellID rPlus1Id(nodesOfCellJ[(rNodesOfCellJ+1+nbNodesOfCellJ)%nbNodesOfCellJ]);
				const gmds::TCellID rMinus1Id(nodesOfCellJ[(rNodesOfCellJ-1+nbNodesOfCellJ)%nbNodesOfCellJ]);
