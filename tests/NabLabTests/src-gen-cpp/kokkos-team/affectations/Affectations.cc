...
const std::pair<size_t, size_t> Affectations::computeTeamWorkRange(const member_type& thread, const size_t& nb_elmt) noexcept
{
	/*
 ...
 * Out variables: e1
 */
void Affectations::computeE1(const member_type& teamMember) noexcept
{
	{
 ...
 */
void Affectations::computeE2(const member_type& teamMember) noexcept
{
	{
		const auto teamWork(computeTeamWorkRange(teamMember, nbCells));
		if (!teamWork.second)
 ...
 * Out variables: e_n0
 */
void Affectations::initE(const member_type& teamMember) noexcept
{
	{
		const auto teamWork(computeTeamWorkRange(teamMember, nbCells));
		if (!teamWork.second)
 ...
 */
void Affectations::initE2(const member_type& teamMember) noexcept
{
	{
		const auto teamWork(computeTeamWorkRange(teamMember, nbCells));
		if (!teamWork.second)
 ...
 */
void Affectations::setUpTimeLoopN(const member_type& teamMember) noexcept
{
	t_n = t_n0;
	for (size_t i1=0; i1<2; i1++)
	{
	...
	do
	{
		...
		Kokkos::parallel_for(team_policy, KOKKOS_LAMBDA(member_type thread)
		{
			...
			if (thread.league_rank() == 0)
				Kokkos::single(Kokkos::PerTeam(thread), KOKKOS_LAMBDA(){updateT();});
			if (thread.league_rank() == 0)
				Kokkos::single(Kokkos::PerTeam(thread), KOKKOS_LAMBDA(){updateU();});
		});
 ...
 */
void Affectations::setUpTimeLoopK(const member_type& teamMember) noexcept
{
	{
		const auto teamWork(computeTeamWorkRange(teamMember, nbCells));
		if (!teamWork.second)
 ...
 */
void Affectations::tearDownTimeLoopK(const member_type& teamMember) noexcept
{
	{
		const auto teamWork(computeTeamWorkRange(teamMember, nbCells));
		if (!teamWork.second)
 ...
 */
void Affectations::updateE(const member_type& teamMember) noexcept
{
	{
		const auto teamWork(computeTeamWorkRange(teamMember, nbCells));
		if (!teamWork.second)
...
void Affectations::simulate()
{
	...
	Kokkos::parallel_for(team_policy, KOKKOS_LAMBDA(member_type thread)
	{
		...
		initE(thread);
		if (thread.league_rank() == 0)
			Kokkos::single(Kokkos::PerTeam(thread), KOKKOS_LAMBDA(){initTandU();});
	});
