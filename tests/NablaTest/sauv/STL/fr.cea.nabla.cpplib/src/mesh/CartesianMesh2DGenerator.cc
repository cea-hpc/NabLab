...
CartesianMesh2DGenerator::generate(int nbXQuads, int nbYQuads, double xSize, double ySize)
{
	...
	for (size_t i(0); i < mesh->m_nodes.size(); ++i)	{
		...
		if (right_node_index_ % nb_x_nodes_!=0)
		  mesh->m_edges[edge_id_++] = move(Edge(static_cast<int>(i), right_node_index_));
		const int below_node_index_(i + nb_x_nodes_);
		if (below_node_index_ < static_cast<int>(mesh->m_nodes.size()))
		  mesh->m_edges[edge_id_++] = Edge(static_cast<int>(i), below_node_index_);
	}
