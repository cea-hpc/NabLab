...
class GitCommitMessageTest
{
    ...
    static val KEYWORDS = List.of("cleanup", "doc", "fix", "releng", "test", "perf") //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$

    static val SIGNED_OFF_BY_PREFIX = "Signed-off-by:" //$NON-NLS-1$

    static val INVALID_GIT_MESSAGE_TITLE = "Invalid Git message title, it should either contain an issue number or one of our regular keywords (cleanup, doc, test, etc)" //$NON-NLS-1$
