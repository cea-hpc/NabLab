...
public abstract class NablaSubNode extends NablaBinaryExpressionNode {
	...
	protected NV1Int add(NV0Int left, Object right, @CachedLibrary("right") NV1IntLibrary arrays) {
		...
		for (int i = 0; i < length; i++) {
			result[i] = Math.subtractExact(leftData, arrays.read(right, i));
		}
	...
	protected NV2Int add(NV0Int left, NV2Int right) {
		...
		for (int i = 0; i < rightData.length; i++) {
			for (int j = 0; j < rightData[0].length; j++) {
				result[i][j] = Math.subtractExact(leftData, rightData[i][j]);
			}
		}
	...
	protected NV1Int add(Object left, NV0Int right, @CachedLibrary("left") NV1IntLibrary arrays) {
		...
		for (int i = 0; i < length; i++) {
			result[i] = Math.subtractExact(arrays.read(left, i), rightData);
		}
	...
	protected NV1Int add(Object left, Object right, //
		...
		for (int i = 0; i < length; i++) {
			result[i] = Math.subtractExact(arraysLeft.read(left, i), arraysRight.read(right, i));
		}
