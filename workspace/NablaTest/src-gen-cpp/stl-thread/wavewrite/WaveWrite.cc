...
void
WaveWrite::Options::jsonInit(const rapidjson::Value::ConstObject& d)
{
	// X_EDGE_LENGTH
...
WaveWrite::WaveWrite(const Options& aOptions, WaveWriteFunctions& aWaveWriteFunctions)
: options(aOptions)
, waveWriteFunctions(aWaveWriteFunctions)
...
/**
 * Job writeToFile called @1.0 in simulate method.
 * In variables: 
 * Out variables: eta
 */
void WaveWrite::writeToFile() noexcept
{
	for (size_t jCells=0; jCells<nbCells; jCells++)
	{
		eta[jCells] = waveWriteFunctions.wavewrite();
	}
}

void WaveWrite::simulate()
{
	std::cout << "\n" << __BLUE_BKG__ << __YELLOW__ << __BOLD__ <<"\tStarting WaveWrite ..." << __RESET__ << "\n\n";
...
int main(int argc, char* argv[]) 
{
	...
	else
	{
		...
		std::cerr << "(WaveWriteDefaultOptions.json)" << std::endl;
		return -1;
	}
	...
	if (d.HasMember("waveWriteFunctions"))
	{
		const rapidjson::Value& valueof_waveWriteFunctions = d["waveWriteFunctions"];
		assert(valueof_waveWriteFunctions.IsObject());
		waveWriteFunctions.jsonInit(valueof_waveWriteFunctions.GetObject());
	}
	
	// simulator must be a pointer if there is a finalize at the end (Kokkos, omp...)
	auto simulator = new WaveWrite(options, waveWriteFunctions);
	simulator->simulate();
