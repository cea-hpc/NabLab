...
private:
	...
	{
    #pragma omp parallel for shared(C_ic)
		for (int jCells = 0; jCells < nbCells; ++jCells)
		{
			int jId(jCells);
			{
	...
	void iniUn() noexcept
	{
    #pragma omp parallel for shared(uj)
		for (int jCells = 0; jCells < nbCells; ++jCells)
		{
			uj[jCells][0] = 0.0;
			uj[jCells][1] = 0.0;
	...
	void iniIc() noexcept
	{
    #pragma omp parallel for shared(rho_ic, p_ic)
		for (int jCells = 0; jCells < nbCells; ++jCells)
		{
	...
	{
    #pragma omp parallel for shared(V_ic)
		for (int jCells = 0; jCells < nbCells; ++jCells)
		{
			int jId(jCells);
    ...
    #pragma omp parallel for reduction(min:reduction1979477213)
		for (int jCells = 0; jCells < nbCells; ++jCells)
    {
      if (reduction1979477213 > deltatj[jCells])
        reduction1979477213 = deltatj[jCells];
    }
    ...
    for (int rInnerNodes = 0; rInnerNodes < nbInnerNodes; ++rInnerNodes)
		{
    ...
    for (int kOuterFaces = 0; kOuterFaces < nbOuterFaces; ++kOuterFaces)
		{
