...
, yc("yc", nbCells)
, U_n("U_n", nbFaces)
, U_nplus1("U_nplus1", nbFaces)
, U_n0("U_n0", nbFaces)
, V_n("V_n", nbFaces)
, V_nplus1("V_nplus1", nbFaces)
, V_n0("V_n0", nbFaces)
, H_n("H_n", nbCells)
, H_nplus1("H_nplus1", nbCells)
, H_n0("H_n0", nbCells)
, Rij_n("Rij_n", nbCells)
, Rij_nplus1("Rij_nplus1", nbCells)
, Rij_n0("Rij_n0", nbCells)
, Fx("Fx", nbCells)
, Fy("Fy", nbCells)
, Dij("Dij", nbCells)
{
...
const std::pair<size_t, size_t> Schema_chap2::computeTeamWorkRange(const member_type& thread, const size_t& nb_elmt) noexcept
{
	/*
 ...
 * Out variables: Dij
 */
void Schema_chap2::initDij(const member_type& teamMember) noexcept
{
	{
 ...
 * Out variables: Fx, Fy
 */
void Schema_chap2::initFxy(const member_type& teamMember) noexcept
{
	{
		const auto teamWork(computeTeamWorkRange(teamMember, nbCells));
		if (!teamWork.second)
 ...
 */
void Schema_chap2::initRij(const member_type& teamMember) noexcept
{
	{
		const auto teamWork(computeTeamWorkRange(teamMember, nbCells));
		if (!teamWork.second)
 ...
 * Out variables: U_n0
 */
void Schema_chap2::initU(const member_type& teamMember) noexcept
{
	{
 ...
 * Out variables: V_n0
 */
void Schema_chap2::initV(const member_type& teamMember) noexcept
{
	{
		const auto innerFaces(mesh->getInnerFaces());
		const size_t nbInnerFaces(innerFaces.size());
 ...
 * Out variables: Xc
 */
void Schema_chap2::initXc(const member_type& teamMember) noexcept
{
	{
		const auto teamWork(computeTeamWorkRange(teamMember, nbCells));
		if (!teamWork.second)
 ...
 */
void Schema_chap2::updateHouter(const member_type& teamMember) noexcept
{
	{
		const auto topCells(mesh->getTopCells());
		const size_t nbTopCells(topCells.size());
		{
			const auto teamWork(computeTeamWorkRange(teamMember, nbTopCells));
			if (!teamWork.second)
				return;
		
			Kokkos::parallel_for(Kokkos::TeamThreadRange(teamMember, teamWork.second), KOKKOS_LAMBDA(const size_t& tcTopCellsTeam)
			{
				int tcTopCells(tcTopCellsTeam + teamWork.first);
	...
	}
	{
		...
		{
			const auto teamWork(computeTeamWorkRange(teamMember, nbBottomCells));
			if (!teamWork.second)
				return;
		
			Kokkos::parallel_for(Kokkos::TeamThreadRange(teamMember, teamWork.second), KOKKOS_LAMBDA(const size_t& bcBottomCellsTeam)
			{
				int bcBottomCells(bcBottomCellsTeam + teamWork.first);
	...
	}
	{
		...
		{
			const auto teamWork(computeTeamWorkRange(teamMember, nbLeftCells));
			if (!teamWork.second)
				return;
		
			Kokkos::parallel_for(Kokkos::TeamThreadRange(teamMember, teamWork.second), KOKKOS_LAMBDA(const size_t& lcLeftCellsTeam)
			{
				int lcLeftCells(lcLeftCellsTeam + teamWork.first);
	...
	}
	{
		...
		{
			const auto teamWork(computeTeamWorkRange(teamMember, nbRightCells));
			if (!teamWork.second)
				return;
		
			Kokkos::parallel_for(Kokkos::TeamThreadRange(teamMember, teamWork.second), KOKKOS_LAMBDA(const size_t& rcRightCellsTeam)
			{
				int rcRightCells(rcRightCellsTeam + teamWork.first);
 ...
 */
void Schema_chap2::updateRij(const member_type& teamMember) noexcept
{
	{
		const auto teamWork(computeTeamWorkRange(teamMember, nbCells));
		if (!teamWork.second)
 ...
 */
void Schema_chap2::updateUinner(const member_type& teamMember) noexcept
{
	{
		...
		{
			const auto teamWork(computeTeamWorkRange(teamMember, nbCompletelyInnerVerticalFaces));
			if (!teamWork.second)
				return;
		
			Kokkos::parallel_for(Kokkos::TeamThreadRange(teamMember, teamWork.second), KOKKOS_LAMBDA(const size_t& civfCompletelyInnerVerticalFacesTeam)
			{
				int civfCompletelyInnerVerticalFaces(civfCompletelyInnerVerticalFacesTeam + teamWork.first);
 ...
 * Out variables: U_nplus1
 */
void Schema_chap2::updateUouter(const member_type& teamMember) noexcept
{
	{
		const auto topCells(mesh->getTopCells());
		const size_t nbTopCells(topCells.size());
 ...
 */
void Schema_chap2::updateVinner(const member_type& teamMember) noexcept
{
	{
		...
		{
			const auto teamWork(computeTeamWorkRange(teamMember, nbCompletelyInnerHorizontalFaces));
			if (!teamWork.second)
				return;
		
			Kokkos::parallel_for(Kokkos::TeamThreadRange(teamMember, teamWork.second), KOKKOS_LAMBDA(const size_t& cihfCompletelyInnerHorizontalFacesTeam)
			{
				int cihfCompletelyInnerHorizontalFaces(cihfCompletelyInnerHorizontalFacesTeam + teamWork.first);
 ...
 * Out variables: V_nplus1
 */
void Schema_chap2::updateVouter(const member_type& teamMember) noexcept
{
	{
		const auto topCells(mesh->getTopCells());
		const size_t nbTopCells(topCells.size());
 ...
 * Out variables: xc, yc
 */
void Schema_chap2::initXcAndYc(const member_type& teamMember) noexcept
{
	{
		const auto teamWork(computeTeamWorkRange(teamMember, nbCells));
		if (!teamWork.second)
 ...
 * Out variables: H_nplus1
 */
void Schema_chap2::updateHinner(const member_type& teamMember) noexcept
{
	{
		const auto innerCells(mesh->getInnerCells());
		const size_t nbInnerCells(innerCells.size());
		{
			const auto teamWork(computeTeamWorkRange(teamMember, nbInnerCells));
			if (!teamWork.second)
				return;
		
			Kokkos::parallel_for(Kokkos::TeamThreadRange(teamMember, teamWork.second), KOKKOS_LAMBDA(const size_t& cInnerCellsTeam)
			{
				int cInnerCells(cInnerCellsTeam + teamWork.first);
 ...
 * Out variables: H_n0
 */
void Schema_chap2::initH1(const member_type& teamMember) noexcept
{
	{
		const auto teamWork(computeTeamWorkRange(teamMember, nbCells));
		if (!teamWork.second)
